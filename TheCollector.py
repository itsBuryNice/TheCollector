#BuryNice 2023 - TheCollector : a simple proxy ripper built in python
import re
import requests
import time
import argparse
...
def test_ports(ip_list, proxy=None):
    ...
    for i, ip in enumerate(ip_list):
        ...
        matches = re.findall(ip_regex, ip)
        if not matches:
            print(f'Skipping invalid IP format: {ip}')
            continue
        ip_with_port = matches[0]
        ip, original_port = re.split(port_regex, ip_with_port)
        for port in COMMON_PORTS:
            ...
            try:
                ...
                if res.status_code == 200:
                    working_proxies.append((proxy_url, port))
                    print(f'{proxy_url} (port: {port}) is working')
                else:
                    print(f'{proxy_url} (port: {port}) returned status code {res.status_code}')
                    
                checked.add((ip, port))
            except requests.exceptions.RequestException as e:
                print(f'{proxy_url} (port: {port}) raised an exception: {e}')
            except Exception as e:
                print(f'An error occurred while testing {proxy_url} (port: {port}): {e}')
...
if __name__ == '__main__':
    ...
    with open('output.txt', 'w') as f:
        for proxy, port in working_proxies:
            f.write(f'{proxy} (port: {port})\n')
            print(f'{proxy} (port: {port}) added to output file')
